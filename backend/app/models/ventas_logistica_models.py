...
class CFDI(Base):
    ...
    # Columnas CFDI 4.0 (migración 0022)
    series_id: Mapped[int | None] = mapped_column(BigInteger, ForeignKey("cfdi_series.series_id"))
    folio: Mapped[int | None] = mapped_column(BigInteger)
    cfdi_version: Mapped[str] = mapped_column(Text, nullable=False, default="4.0")
    issuer_config_id: Mapped[int | None] = mapped_column(BigInteger, ForeignKey("cfdi_issuer_config.config_id"))
    # Snapshot fiscal del emisor al momento del timbrado
    issuer_rfc: Mapped[str | None] = mapped_column(Text)
    issuer_name: Mapped[str | None] = mapped_column(Text)
    issuer_tax_regime: Mapped[str | None] = mapped_column(Text)
    # Snapshot fiscal del receptor al momento del timbrado
    receiver_rfc: Mapped[str | None] = mapped_column(Text)
    receiver_name: Mapped[str | None] = mapped_column(Text)
    receiver_tax_regime: Mapped[str | None] = mapped_column(Text)
    receiver_zip: Mapped[str | None] = mapped_column(Text)
    # Timbre Fiscal Digital devuelto por el PAC
    ...
    timbre_date: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
...
class CFDIItem(Base):
    ...
    # Claves SAT CFDI 4.0 con FK (migración 0022)
    sat_product_key_id: Mapped[UUID | None] = mapped_column(PGUUID(as_uuid=True), ForeignKey("sat_product_keys.id"))
    sat_unit_key_id: Mapped[UUID | None] = mapped_column(PGUUID(as_uuid=True), ForeignKey("sat_unit_keys.id"))
    description: Mapped[str] = mapped_column(Text, nullable=False)
    unit_price: Mapped[float] = mapped_column(Numeric(14, 4), nullable=False, default=0)
    discount_amount: Mapped[float] = mapped_column(Numeric(14, 4), nullable=False, default=0)
    iva_pct: Mapped[float] = mapped_column(Numeric(6, 4), nullable=False, default=0.16)
    tax_rate: Mapped[float] = mapped_column(Numeric(6, 4), nullable=False, default=0.16)
    subtotal: Mapped[float] = mapped_column(Numeric(14, 4), nullable=False, default=0)
    tax_amount: Mapped[float] = mapped_column(Numeric(14, 4), nullable=False, default=0)
    total: Mapped[float] = mapped_column(Numeric(14, 4), nullable=False, default=0)
...
class CFDICreditNote(Base):
    ...
    relation_type: Mapped[str | None] = mapped_column(String(2))  # c_TipoRelacion SAT
