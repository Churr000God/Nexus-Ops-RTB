...
.PARAMETER WithFrontend
    También levanta el contenedor frontend.
...
# ── Paso 3: Docker — Redis + Backend ─────────────────────────────────────────
$Step++
Write-Step $Step $TotalSteps "Docker Compose — Redis + Backend"
...
# ── Paso opcional: Frontend ───────────────────────────────────────────────────
if ($WithFrontend) {
    $Step++
    Write-Step $Step $TotalSteps "Docker Compose — Frontend"
    docker compose up -d --build frontend
    if ($LASTEXITCODE -ne 0) {
        Write-Warn "Frontend falló al levantarse (exit $LASTEXITCODE)."
    } else {
        Write-Ok "Frontend levantado."
    }
}
